...
bool StereoCameraTara::openCamera(){
    if (isConnected()){
        closeCamera();
    }
...
bool StereoCameraTara::closeCamera(){
    if (connected){
        if (camera.isOpened()) {
            camera.release();
...
bool StereoCameraTara::captureSingle(){
    bool res = false;
...
void StereoCameraTara::captureThreaded(){
    future = QtConcurrent::run(this, &StereoCameraTara::captureSingle);
}

bool StereoCameraTara::enableCapture(bool enable){
    if (enable){
...
std::string StereoCameraTara::get_device_path_serial(IMoniker *pMoniker){
...
void StereoCameraTara::openHID(void) {
    hid_init();
...
qint64 StereoCameraTara::getSerial() {
...
int StereoCameraTara::usb_index_from_serial(std::string serial){
    // Create the System Device Enumerator.
    CoInitialize(NULL);
    ICreateDevEnum *pDevEnum;
    IEnumMoniker *pEnum;
...
std::string StereoCameraTara::serial_from_usb_index(int index){
    // Create the System Device Enumerator.
    CoInitialize(NULL);
    ICreateDevEnum *pDevEnum;
    IEnumMoniker *pEnum;
...
std::string StereoCameraTara::serial_from_device_path(std::string usb_device_path){
...
int StereoCameraTara::getExposure() {
    /*
...
bool StereoCameraTara::enableAutoExposure(bool enable) {
    if (enable)
...
bool StereoCameraTara::setExposure(double exposure_milliseconds) {
...
bool StereoCameraTara::setFPS(int fps){
    if (!isCapturing()){
        double fps_d;
        if (fps == 30){
            frame_rate = 30;
...
bool StereoCameraTara::enableHDR(bool enable){
    if (deimos_device == NULL) return false;
...
bool StereoCameraTara::send_hid(std::vector<unsigned char> &buffer,
                                  size_t command_len) {
    if (deimos_device == NULL) return false;
...
bool StereoCameraTara::setFrameSize(int width, int height) {
    bool res = false;
...
bool StereoCameraTara::setFrame16(void) {
    bool res = false;
...
void StereoCameraTara::getFrameRate() {
...
std::string StereoCameraTara::wchar_to_string(WCHAR * buffer) {
    std::wstring ws(buffer);
    std::string buffer_s(ws.begin(), ws.end());
    return buffer_s;
}

StereoCameraTara::~StereoCameraTara(void) {
    if (connected){
        closeCamera();
    }
}
